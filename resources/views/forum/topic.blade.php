                ...
                <section class='d-flex flex-row post-forum'>
                    <aside class='d-flex flex-column align-items-center forum-infos'>
                        ...
                        @if (Auth::user()->isAdministrator() || Auth::user()->isModerator())
                            <a href="{{ route('ban_user', ['id' => $message->user->id ]) }}" type='button'><i class="fas fa-user-slash"></i></a>
                        @endif
                        <dl class='border-top'>
                    ...
                    <article class='message-forum'>
                        ...
                        @if ($message->user == Auth::user() || Auth::user()->isAdministrator() || Auth::user()->isModerator())
                            <div class="d-flex flex-row btn-crud-forum">
                                <a href="" type='button' class='btn btn-outline-warning'><i class='far fa-edit'></i></a>
                                <a href="{{ route('delete_message', ['topic' => $message->topic->id, 'id' => $message->id]) }}" type='button' class='btn btn-outline-danger'><i class='far fa-trash-alt'></i></a>
                            </div>    
                        @endif
                        @endauth
                        {!! $message->message !!}
